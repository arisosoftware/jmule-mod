...
public class NewFilesDialg extends NewED2KDialog {
	...
	/*
	 * class Pair { public Pair(ED2KFileLink fileLink, boolean isSelected) {
	 * this.fileLink = fileLink; this.isSelected = isSelected; } public
	 * Pair(ED2KFileLink fileLink) { this.fileLink = fileLink; this.isSelected =
	 * true; } public ED2KFileLink fileLink; public boolean isSelected; }
	 */
	...
	// private final Map<ED2KFileLink,Boolean> selected_list = new
	// HashMap<ED2KFileLink,Boolean>();
	...
	class NewFilesTableModel extends AbstractTableModel {
		...
		public Object getValueAt(int row, int col) {
			...
			switch (col) {
			case FILE_NAME:
				return file_link.getFileName();
			case FILE_SIZE:
				return file_link.getFileSize();
			case FILE_HASH:
				return file_link.getFileHash();
			}
			return null;
		}
	...
	private void init() {
		...
		// ------------------------------------------------------------------
		central_panel.add(paste_ed2k_links, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.NORTH,
				GridBagConstraints.HORIZONTAL, new Insets(0, 20, 0, 20), 0, 0));
		central_panel.add(learn_about_ed2k_links_label, new GridBagConstraints(0, 2, 1, 1, 0.0, 0.0,
		...
		// table.getColumnModel().getColumn(0).setCellEditor(new DefaultCellEditor(new
		// JCheckBox()));
		/*
		 ...
		 * row, column); final JCheckBox check_box = new JCheckBox();
		 * //check_box.setSelected(true); final ED2KFileLink file_link =
		 * (ED2KFileLink)value; boolean is_checked = selected_list.get(file_link);
		 * check_box.setSelected(true); //if(pair.isSelected)
		 * check_box.setSelected(true); //check_box.setSelected(true); //
		 * check_box.addMouseListener(new MouseAdapter() { // public void
		 * mouseClicked(MouseEvent evt) { if(is_checked) {
		 * selected_list.remove(file_link); selected_list.put(file_link,false);
		 * check_box.setSelected(false); } else { selected_list.remove(file_link);
		 * selected_list.put(file_link,true); check_box.setSelected(true); } // } //});
		 * check_box.setText(file_link.getFileName());
		 * check_box.setToolTipText(file_link.getFileName()); if(isSelected) {
		 * check_box.setForeground(table.getSelectionForeground());
		 * check_box.setBackground(table.getSelectionBackground()); } else {
		 * check_box.setForeground(table.getForeground());
		 * check_box.setBackground(table.getBackground()); } return check_box; } });
		 */
		//
				...
				(new JMThread(new JMRunnable() {
					public void JMRun() {
						try {
							...
							List<ED2KFileLink> clipboard_file_links = ED2KFileLink
							...
							SwingUtilities.invokeAndWait(new Runnable() {
								public void run() {
									if (ed2k_file_links_list.size() != 0)
										filesPanel();
								}
							});
						} catch (Throwable t) {
	...
	private void filesPanel() {
		...
		central_panel.add(paste_ed2k_links, new GridBagConstraints(0, 1, 2, 1, 0.0, 0.0, GridBagConstraints.NORTH,
				GridBagConstraints.HORIZONTAL, new Insets(10, 10, 0, 10), 0, 0));
		central_panel.add(scroll_panel, new GridBagConstraints(0, 0, 4, 1, 0.0, 0.0, GridBagConstraints.CENTER,
