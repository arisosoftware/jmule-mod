 ...
 *          $$Date: 2010/05/15 18:19:58 $$
 */
public class MainWindow extends JFrame implements WindowListener {
	private JMenuBar main_menu_bar;
	private JMenu file, fnew, view, tools, help, tabs;
	private JMenuItem new_downloads, new_servers, exit, import_server_list, ui_chooser, config_wizard, options,
			open_support, project_forums, bug_tracker, check_for_updates, about;
	...
	public MainWindow() {
		...
		_server_manager.addServerListListener(new ServerManagerListener() {
			...
			public void connected(Server server) {
				...
				status_bar.setClientIDType(
						(server.getClientID().isHighID() ? ClientIDTypes.HIGH_ID : ClientIDTypes.LOW_ID),
						server.getClientID().toString());
					...
					SwingUtilities.invokeLater(new Runnable() {
						...
						public void run() {
							JOptionPane.showMessageDialog(_this,
									Lang.getString("mainwindow.not_enough_space_dialog.message", nes.getFileName(),
											FileFormatter.formatFileSize(nes.getTotalSpace()),
											FileFormatter.formatFileSize(nes.getFreeSpace())),
									Lang.getString("mainwindow.not_enough_space_dialog.title"),
	...
	public void initUIComponents() {
		...
		// ImageIcon(getClass().getClassLoader().getResource("resources/jmule.png")).getImage());
	...
	private void setMainMenu() {
		...
		status_bar_visibility.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if (status_bar_visibility.isSelected())
		...
		tool_bar_visibility.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if (tool_bar_visibility.isSelected())
					main_buttons_bar.setVisible(true);
				else
					main_buttons_bar.setVisible(false);
			}
		});
						...
						public void JMRun() {
							try {
								...
								SwingUtilities.invokeLater(new Runnable() {
									public void run() {
										JOptionPane.showMessageDialog(_this,
												"An error occured, most probably the file format is wrong",
	...
	public enum ServerStatusTypes {
		DISCONNECTED {
			public String toString() {
				return Lang.getString("mainwindow.statusbar.label.disconnected");
			}
		},
		CONNECTING {
			public String toString() {
				return Lang.getString("mainwindow.statusbar.label.connecting");
			}
		},
		CONNECTED {
			public String toString() {
				return Lang.getString("mainwindow.statusbar.label.connected");
			}
		};
	...
	public enum ClientIDTypes {
		LOW_ID {
			public String toString() {
				return Lang.getString("mainwindow.statusbar.label.low_id");
			}
		},
		HIGH_ID {
			public String toString() {
				return Lang.getString("mainwindow.statusbar.label.high_id");
			}
		},
	...
	class StatusBar extends JPanel implements Refreshable {
		...
		public void setServerStatus(ServerStatusTypes statusType, String server) {
			switch (statusType) {
			case DISCONNECTED:
				server_status.setText(ServerStatusTypes.DISCONNECTED.toString());
				server_status.setToolTipText("");
				server_status.setIcon(ImgRep.getIcon("toolbar_disconnected.png"));
				break;
			case CONNECTING:
				server_status.setText(ServerStatusTypes.CONNECTING.toString());
				server_status.setToolTipText("Connecting to " + server);
				break;
			case CONNECTED:
				server_status.setText(ServerStatusTypes.CONNECTED.toString());
				server_status.setToolTipText(server);
				server_status.setIcon(ImgRep.getIcon("toolbar_connected.png"));
				break;
			}
		}
		...
		public void setClientIDType(ClientIDTypes clientIDType, String clientID) {
			switch (clientIDType) {
			case LOW_ID:
				client_id.setForeground(Color.RED);
				client_id.setText("LOW ID");
				client_id.setToolTipText(clientID);
				break;
			case HIGH_ID:
				client_id.setForeground(Color.BLACK);
				client_id.setText("HIGH ID");
				client_id.setToolTipText(clientID);
				break;
			case NO_ID:
				client_id.setText("");
				client_id.setToolTipText("");
				break;
			}
		}
		...
		public void refresh() {
			try {
				...
				download_speed.setText((download_speed_limit != "" ? "[" : "") + download_speed_limit
						+ (download_speed_limit != "" ? "] " : "") + downloadSpeed + " ");
				upload_speed.setText((upload_speed_limit != "" ? "[" : "") + upload_speed_limit
						+ (upload_speed_limit != "" ? "] " : "") + uploadSpeed + " ");
	...
	private void setStatusBar() {
		...
		try {
			if (_config.getDownloadLimit() != 0)
				status_bar.setDownloadSpeedLimit(SpeedFormatter.formatSpeed(_config.getDownloadLimit()));
			if (_config.getUploadLimit() != 0)
				status_bar.setUploadSpeedLimit(SpeedFormatter.formatSpeed(_config.getUploadLimit()));
		} catch (Throwable cause) {
			cause.printStackTrace();
		}
	...
	private void shutdownTheSystem() {
		if (_pref.isPromptOnExitEnabled()) {
			int value = JOptionPane.showConfirmDialog(null, "Are you sure you want to exit ?", "JMule",
