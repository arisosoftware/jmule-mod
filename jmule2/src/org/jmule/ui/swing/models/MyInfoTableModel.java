...
public class MyInfoTableModel extends AbstractTableModel {
	...
	private final static String row_names[] = { "Status", "Nickname", "IP", "ID", "Shared files" };
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		...
		try {
			...
			if (columnIndex == 1) {
				switch (rowIndex) {
				case STATUS:
					return _server_manager.isConnected() ? "Connected" : "Disconnected";
				case NICKNAME:
					return _config_manager.getNickName();
				case IP:
					return _server_manager.isConnected()
							? _server_manager.getConnectedServer().getClientID().getAsString()
							: " ";
				case ID:
					boolean is_connected = _server_manager.isConnected();
					ClientID client_id = is_connected ? _server_manager.getConnectedServer().getClientID() : null;
					return is_connected ? (client_id.toString() + (client_id.isHighID() ? " (High)" : " (Low)")) : " ";
				case SHARED_FILES:
					return _sharing_manager.getFileCount() + " ";
				}
			}
