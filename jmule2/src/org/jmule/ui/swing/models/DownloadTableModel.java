...
public class DownloadTableModel extends AbstractTableModel {
	...
	private final static String column_names[] = {
			// "#",
			"File name", "File size", "Transferred", "Download speed", "Upload speed", "Pieces", "Done",
			"Complete sources", "Partial sources", "ETA", "Status", "Progress" };

	public DownloadTableModel() {
		/*
		 * _download_manager.addDownloadManagerListener(new DownloadManagerListener() {
		 * public void downloadAdded(FileHash fileHash) { downloads_order.add(fileHash);
		 * } public void downloadStopped(FileHash fileHash) {
		 * downloads_order.remove(fileHash); } public void downloadRemoved(FileHash
		 * fileHash) {} public void downloadStarted(FileHash fileHash) {}
		 * 
		 * });
		 */
	}
