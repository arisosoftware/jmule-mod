...
public class SharedTab extends AbstractTab {
	...
	class SharedFoldersListModel extends AbstractListModel {
		...
		public Object getElementAt(int index) {
			if (index == 0)
				return FilesCategory.INCOMING_FILES;
			if (index == 1)
				return FilesCategory.INCOMPLETE_FILES;
			if (index == 2)
				return FilesCategory.ALL_FILES;
			Object result = null;
			try {
		...
		public int getSize() {
			int result = 0;
			try {
				if (_config_manager.getSharedFolders() == null)
					result = 3;
	...
	class TopPanel extends JPanel {
		...
		private void init() {
			...
			this.add(add_button, new GridBagConstraints(1, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
					GridBagConstraints.HORIZONTAL, new Insets(0, 10, 0, 10), 0, 0));
			this.add(remove_button, new GridBagConstraints(2, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
					GridBagConstraints.HORIZONTAL, new Insets(0, 10, 0, 10), 0, 0));
			this.add(clear_button, new GridBagConstraints(3, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
					GridBagConstraints.HORIZONTAL, new Insets(0, 10, 0, 10), 0, 0));
			this.add(rescan_button, new GridBagConstraints(4, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
					GridBagConstraints.HORIZONTAL, new Insets(0, 10, 0, 10), 0, 0));
		...
		file_chooser.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if (event.getActionCommand() == JFileChooser.APPROVE_SELECTION) {
					...
					final List<File> already_existed_folders = org.jmule.core.utils.FileUtils
							.extractNewFolders(current_chosen_folders, chosen_folders.getFoldersList(), newFolders);
		...
		_updater.addRefreshable(new Refreshable() {
			public void refresh() {
				...
				if (shared_file != null) {
					...
					str = " [Hashing : " + shared_file.getSharingName() + " ("
							+ NumberFormatter.formatProgress(percent_complete) + ")]";
			...
			public void run() {
				...
				if (object instanceof FilesCategory) {
					...
					case INCOMPLETE_FILES:
						for (PartialFile partial_file : _sharing_manager.getPartialFiles())
							shared_files.add(partial_file);
						break;
					case INCOMING_FILES:
						list_of_files = FileUtils
								.traverseDirAndReturnListOfFiles(new File(ConfigurationManager.INCOMING_DIR));
						// i = FileUtils.iterateFiles(new File(ConfigurationManager.INCOMING_DIR), null,
						// true);
						is_iterator = true;
					}
