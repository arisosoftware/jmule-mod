...
public class SearchTab extends AbstractTab implements SearchResultListener {
	...
	final CloseableTabbedPane tabbed_panel = new CloseableTabbedPane(); // each search result is located on this tabbed
																		// panel
	...
	public SearchTab(final JFrame parent) {
		...
		top_panel.add(search_field, new GridBagConstraints(1, 1, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
		start_button = new JButton();
		top_panel.add(start_button, new GridBagConstraints(2, 1, 1, 1, 0.0, 0.0, GridBagConstraints.EAST,
				GridBagConstraints.BOTH, new Insets(0, 7, 0, 0), 0, 0));
		start_button.setText("Start");
		search_label = new JLabel();
		top_panel.add(search_label, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
		...
		start_button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if (!_server_manager.isConnected()) {
					JOptionPane.showMessageDialog(null, "The system is not connected to server", "Warning",
	...
	public void resultArrived(SearchResult searchResult) {
		for (Object[] obj : search_results_panels) {
			if (searchResult.getSearchQuery().getQuery().equalsIgnoreCase((String) obj[0])) {
