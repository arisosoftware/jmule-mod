...
public class CloseableTabbedPane extends JTabbedPane implements MouseListener, MouseMotionListener {
					...
					if (e.getID() == e.MOUSE_CLICKED) {
						...
						if (fireCloseTab(selIndex)) {
							if (selIndex > 0) {
								...
								MouseEvent event = new MouseEvent((Component) e.getSource(), e.getID() + 1,
										System.currentTimeMillis(), e.getModifiers(), rec.x, rec.y, e.getClickCount(),
	 ...
	 * 
	 * @return all of the <code>SearchListener</code>s added or an empty array if no
	...
	class CloseTabIcon implements Icon {
		...
		/**
		 * Gets the bounds of this icon in the form of a <code>Rectangle<code> object.
		 * The bounds specify this icon's width, height, and location relative to its
		 * parent.
		 * 
