			...
			if ((offs >= 0) && (offs <= textBuffer.length())) {
				...
				if (textBuffer.length() > maxLength) {
					JOptionPane.showMessageDialog(IntegerTextField.this,
							"The number of characters 	must be less than or equal to " + getMaxLength(),
				...
				if (str.equals(".") && isIPField) {
					...
					try {
						...
						if ((typedValue > maxValue) || (typedValue < minValue)) {
							JOptionPane.showMessageDialog(IntegerTextField.this,
									"The value can only be from " + getMinValue() + " to " + getMaxValue(),
									"Error Message", JOptionPane.ERROR_MESSAGE);
						} else {
							super.insertString(offs, new String(str), a);
						}
						...
						JOptionPane.showMessageDialog(IntegerTextField.this, "Only numeric values allowed.",
								"Error Message", JOptionPane.ERROR_MESSAGE);
					}
				}
			}
