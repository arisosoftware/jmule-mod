...
public class PartFileRenderer extends Canvas {
	...
	public void paint(Graphics g) {
		...
		g.setColor(Color.WHITE);
		/*
		 * for(Gap gap : gap_list.getGaps()) { g.fillRect(
		 * Math.round(gap.getStart()*pixels_for_byte), 7,
		 * Math.round(gap.getEnd()*pixels_for_byte - gap.getStart()*pixels_for_byte),
		 * this.getHeight()); }
		 */
		for (Pair p : pairs) {
			g.fillRect(Math.round(p.getX() * pixels_for_byte), 7,
					Math.round(p.getY() * pixels_for_byte - p.getX() * pixels_for_byte), this.getHeight());
			// ---------------------------------------------------------
