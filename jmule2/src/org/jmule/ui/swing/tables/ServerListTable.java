 ...
 * 
 * @author javajox
 * @version $$Revision: 1.7 $$ Last changed by $$Author: javajox $$ on $$Date:
 *          2009/09/27 14:20:00 $$
 */
public class ServerListTable extends JMTable {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
			...
			if (!server.isDown())
				if (server.isConnected())
					this.setIcon(ImgRep.getIcon("server_connected.png"));
				else
					this.setIcon(ImgRep.getIcon("server.png"));
			else
				this.setIcon(ImgRep.getIcon("server_error.png"));
			...
			String ping = (server.getPing() > 0) ? "<tr><td>Ping</td><td>" + server.getPing() + "</td></tr>" + ""
					: null;
			String users = (server.getNumUsers() > 0)
					? "<tr><td>Users</td><td>" + NumberFormatter.formatSizeHumanReadable(server.getNumUsers())
							+ " (max " + NumberFormatter.formatSizeHumanReadable(server.getMaxUsers()) + ")"
							+ "</td></tr>" + ""
					: null;
			// String max_users = ( server.getMaxUsers() > 0 ) ? server.getMaxUsers() + "" :
			// null;
			String files = (server.getNumFiles() > 0)
					? "<tr><td>Files</td><td>" + NumberFormatter.formatSizeHumanReadable(server.getNumFiles())
							...
							+ "</td></tr>" + ""
					: null;
			// String soft_limit = ( server.getSoftLimit() > 0 ) ? server.getSoftLimit() +
			// "" : null;
			// String hard_limit = ( server.getHardLimit() > 0 ) ? server.getHardLimit() +
			// "" : null;
			// String down = ( server.isDown() ? "<tr><td>Status</td><td><font
			// color=\"red\"><b>DOWN</b></font></td></tr>" : null );

			this.setText(" " + server.getName());
			String text = "<html>" + "<body " + (server.isDown() ? "color=\"#c0c0c0\"" : "") + ">"
					+ (name != null ? name : "") + (ver != null ? ver : "") + (desc != null ? desc : "") + "<hr>"
					+ "<table>"
					+ ((ip != null || flag_icon_url != null)
							? ("<tr><td>IP</td><td>"
									+ (flag_icon_url != null ? "<img src=\"" + flag_icon_url + "\">&nbsp;" : "")
									+ (ip != null ? ip : "") + "</td></tr>")
							: "")
					+ (ping != null ? ping : "") + (users != null ? users : "") + (files != null ? files : "")
					+ "</table>" + "</body>" + "</html>";
	...
	private void init() {
		...
		class PopupListener extends MouseAdapter {
			JMenuItem connect_to, disconnect, add, remove_selected, remove_all, properties, add_to_static_list,
					remove_from_static_list, paste_ed2k_links, copy_ed2k_links, column_setup;
			...
			public PopupListener() {
				...
				add.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if (server_dialog.getDialogAction() == IDialog.DialogAction.OK) {
							try {
								_server_manager.newServer(server_dialog.getServerIP(), server_dialog.getServerPort());
				...
				add_to_static_list = new JMenuItem(
						Lang.getString("mainwindow.serverlisttab.serverlist.popupmenu.add_to_static_list"),
				...
				remove_from_static_list = new JMenuItem(
						Lang.getString("mainwindow.serverlisttab.serverlist.popupmenu.remove_from_static_list"),
					...
					public void actionPerformed(ActionEvent e) {
						try {
							...
							List<ED2KServerLink> server_links = ED2KServerLink
									.extractLinks(clipboard_contents.toString());
						...
						clipboard.setContents(new Transferable() {
							public Object getTransferData(DataFlavor flavor)
								...
								for (Server server : selected_servers) {
									ed2k_links += server.getServerLink().getAsString()
				...
				column_setup.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						for (TableColumnExt column : table_columns) {
							if (column.isVisible())
								col_set_rows_cols[i][0] = true;
							else
								col_set_rows_cols[i][0] = false;
						...
						col_set_table.setModel(new DefaultTableModel(col_set_rows_cols,
								new String[] { "Visibility", "Name", "Description" }) {
			...
			private void showPopup(MouseEvent e) {
				if (e.isPopupTrigger()) {
					...
					// int[] selected_rows = ( _server_manager.getServersCount() !=0 ) ?
					// getSelectedRows() : new int[0];
	...
	private enum ServersOp {
		MAKE_SELECTED_STATIC, MAKE_SELECTED_NON_STATIC, REMOVE_SELECTED_FROM_LIST, REMOVE_ALL_FROM_LIST, CONNECT_TO
	}
	...
	private void executeSelectedServersOp(ServersOp operation) {
		...
		(new JMThread(new JMRunnable() {
			public void JMRun() {
				...
				Server[] selected_servers = getSelectedServers();
				// if the foperation == ServersOp.CONNECT_TO we suppose that only one server is
				// selected
				...
				for (Server server : selected_servers) {
					try {
						switch (foperation) {
						case MAKE_SELECTED_STATIC:
							if (!server.isStatic())
								server.setStatic(true);
							break;
						case MAKE_SELECTED_NON_STATIC:
							if (server.isStatic())
								server.setStatic(false);
							break;
						case REMOVE_SELECTED_FROM_LIST:
							_server_manager.removeServer(server);
							break;
						}
					} catch (Throwable cause) {
						cause.printStackTrace();
	...
	// we override the getSelectedRows from JTable because it returns the last
	// selected rows when the table has 0 rows
	// if the JTable is cleared, it should return 0 selected rows, maybe a bug or my
	// misunderstanding
	 ...
	 * -----------------------------------------------------------------------------
	 * ------- ONE_SELECTED | A D G | MULTIPLE_SELECTED | B E H | VOID_LIST | * F *
	 * |
	 * 
	 * A,B,D,E,F,G,H - conditions * - can't be
	 * 
	 * Add Ac1 Remove selected Ac2 Remove all Ac3 Copy ED2K Links Ac4 Paste ED2K
	 * Links Ac5 Connect Ac6 Disconnect Ac7 Column setup Ac8 Properties Ac9
	 * 
	 * | Ac1 Ac2 Ac3 Ac4 Ac5 Ac6 Ac7 Ac8 Ac9
	 * -------------------------------------------------------------- A | Yes Yes
	 * Yes Yes Yes Yes Yes Yes Yes | B | Yes Yes Yes Yes Yes - Yes Yes - | D | Yes
	 * Yes Yes Yes Yes Yes - Yes Yes | E | Yes Yes Yes Yes Yes - - Yes - | F | Yes -
	 * - - Yes - - Yes - | G | Yes Yes Yes Yes Yes - Yes Yes Yes | H | Yes Yes Yes
	 * Yes Yes - Yes Yes - |
	 */
