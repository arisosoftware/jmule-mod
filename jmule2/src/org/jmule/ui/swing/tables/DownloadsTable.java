...
public class DownloadsTable extends JMTable {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
			...
			try {
				if (_upload_manager.hasUpload(session.getFileHash()))
					...
					u_speed = SpeedFormatter.formatSpeed(0);
			...
			String str = "<html> " + "<table><tr><td><img src=\"" + icon_url + "\"></td><td><b>" + fn
					+ "</b></td><tr></table>" + "<hr>" + "<table>" + "<tr><td>Size</td><td>" + size + "</td></tr>"
					+ "<tr><td>Done</td><td>" + done + "</td></tr>" + "<tr><td>Down speed</td><td>" + d_speed
					+ "</td></tr>" + "<tr><td>Up speed</td><td>" + u_speed + "</td></tr>" + "<tr><td>ETA</td><td>" + eta
					+ "</td></tr>" + "</table>" + "</html>";
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
			...
			try {
				if (_upload_manager.hasUpload(session.getFileHash()))
					...
					this.setText(SpeedFormatter.formatSpeed(0) + " ");
			} catch (Throwable cause) {
				cause.printStackTrace();
			}
			return this;
	...
	class PiecesTableCellRenderer extends JPanel implements TableCellRenderer { // extends DefaultTableCellRenderer {
		...
		public void paint(Graphics g) {
			...
			// g.fillRect(60+this.getWidth()-300+50, 0, this.getWidth()-300-120,
			// this.getHeight());
			...
			/*
			 * for(Pair p : pairs) { g.fillRect( Math.round(p.getX()*pixels_for_byte), 5,
			 * Math.round(p.getY()*pixels_for_byte - p.getX()*pixels_for_byte),
			 * this.getHeight()); //
			 * ---------------------------------------------------------
			 * System.out.println(Math.round(p.getX()*pixels_for_byte));
			 * System.out.println(Math.round(p.getY()*pixels_for_byte -
			 * p.getX()*pixels_for_byte)); }
			 */
		}
	...
	class StatusTableCellRenderer extends DownloadTableCellRenderer {
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
			...
			else if (session.getStatus() == DownloadStatus.STOPPED)
				this.setText(" stopped");
			return this;
		}
	}

	class ProgressBarTableCellRenderer extends JProgressBar implements TableCellRenderer { // extends
	...
	private void init() {
		...
		class PopupListener extends MouseAdapter {
			...
			JMenuItem start_download, stop_download, cancel_download, paste_ed2k_links, copy_ed2k_links, column_setup,
					properties;
						...
						clipboard.setContents(new Transferable() {
							public Object getTransferData(DataFlavor flavor)
								...
								for (DownloadSession session : download_sessions) {
									ed2k_links += session.getED2KLink().getAsString()
	 ...
	 * -----------------------------------------------------------------------------
	 * ---------- ONE_SELECTED | A C * | MULTIPLE_SELECTED | B D F | VOID_LIST | * *
	 * * |
	 * 
	 * E - void list * - can't be
	 * 
	 * Ac1 | Start download Ac2 | Stop download Ac3 | Cancel download Ac4 | Copy
	 * ed2k link(s) Ac5 | Paste ed2k link(s) Ac6 | Column setup Ac7 | Details
	 * 
	 * | Ac1 Ac2 Ac3 Ac4 Ac5 Ac6 Ac7
	 * -------------------------------------------------------- A | - Yes Yes Yes
	 * Yes Yes Yes | B | - Yes Yes Yes Yes Yes Yes | C | Yes - Yes Yes Yes Yes Yes |
	 * D | Yes - Yes Yes Yes Yes Yes | E | - - - - Yes Yes - | F | Yes Yes Yes Yes
	 * Yes Yes -
	 */
	...
	private ConditionType whichCondition() {
		...
		if ((download_sessions.length == 1) && (download_sessions[0].getStatus() == DownloadStatus.STARTED))
		...
		if ((download_sessions.length == 1) && (download_sessions[0].getStatus() == DownloadStatus.STOPPED))
