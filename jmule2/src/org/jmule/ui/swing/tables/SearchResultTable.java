...
public class SearchResultTable extends JMTable {
	...
	public SearchResultTable(JFrame parent, SearchResult searchResult, JTabbedPane tabbedPanel,
			SearchResultPanel search_result_pane) {
		this.parent = parent;
	...
	private void init() {
		...
		class PopupListener extends MouseAdapter {
			JMenuItem start_download, try_again, copy_ed2k_links, close, column_setup, properties;
						...
						clipboard.setContents(new Transferable() {
							public Object getTransferData(DataFlavor flavor)
								...
								for (SearchResultItem result_item : result_items) {
									ed2k_links += result_item.getAsED2KLink().getAsString()
	 ...
	 * =============================================================================
	 * ============================================================================
	 * | | Downloading | On Shared | Downloading & New files | Shared & New files |
	 * Shared & Downloading & New files | New Files |
	 * =============================================================================
	 * ============================================================================
	 ...
	 * |----------------------------------------------------------------------------
	 * ----------------------------------------------------------------------------
	 * J - no search results A,B,C,D,E,F,G,H,I,J - conditions X - can't be
	 * 
	 * Start download Ac1 Try again Ac2 Copy ED2K Links Ac3 Close Ac5 Column setup
	 * Ac6 Properties Ac7
	 * 
	 * | Ac1 Ac2 Ac3 Ac5 Ac6 Ac7 -------------------------------------------- A | -
	 * yes yes yes yes yes | B | - yes yes yes yes - | C | - yes yes yes yes yes | D
	 * | - yes yes yes yes - | E | yes yes yes yes yes - | F | yes yes yes yes yes -
	 * | G | yes yes yes yes yes - | H | yes yes yes yes yes yes | I | yes yes yes
	 * yes yes - | J | - - - yes yes -
	 * 
	 */
	...
	private ConditionType whichCondition() {
		...
		if ((selected_rows.length == 1) && _download_manager.hasDownload(results[0].getFileHash()))
		...
		if (selected_rows.length > 1) {
			...
			for (SearchResultItem result : results) {
				if (_download_manager.hasDownload(result.getFileHash()))
					some_downloading = true;
				if (!_sharing_manager.hasFile(result.getFileHash())
						&& !_download_manager.hasDownload(result.getFileHash()))
					some_new = true;
			}
			if (some_downloading && some_new)
		...
		if (selected_rows.length > 1) {
			...
			if (some_shared && some_downloading && some_new)
				return ConditionType.G;
		}
		if ((selected_rows.length == 1) && (!_sharing_manager.hasFile(results[0].getFileHash())
				&& !_download_manager.hasDownload(results[0].getFileHash())))
	...
	public List<SearchResultItem> getFilesByStatus(FileStatus fileStatus) {
		...
		for (SearchResultItem search_result_item : selected_results) {
			switch (fileStatus) {
			case SHARING:
				if (_sharing_manager.hasFile(search_result_item.getFileHash()))
					result.add(search_result_item);
				break;
			case DOWNLOADING:
				if (_download_manager.hasDownload(search_result_item.getFileHash()))
					result.add(search_result_item);
				break;
			case NEW:
				if (!_download_manager.hasDownload(search_result_item.getFileHash())
