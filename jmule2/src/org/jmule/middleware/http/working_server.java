...

{
	...
	public void run() {
		...
		try { // Begin reading request
			...
			/*
			 * byte [] temp; String st; st = dataFromClient.readLine();
			 * System.out.println(st); st = st+"\r\n"; temp = st.getBytes();
			 * dataToClient.write(temp); st = "";
			 */
	...
	public void checkMethod(String meth) throws IOException {
		...
		if (((meth.equals("GET")) || (meth.equals("POST")) || (meth.equals("HEAD")))) {
			validMethod = true;
		} else if (((meth.equals("OPTIONS")) || (meth.equals("PUT")) || (meth.equals("DELETE"))
				|| (meth.equals("TRACE")))) {
			validMethod = false;
						...
						if ((quesE != -1) || (ques.indexOf(".cgi") != -1)) {
							...
							if (staticFile.isDirectory()) {
								...
								if (!rqline.hasMoreTokens()) {
									File defStaticFile = new File(
											_serverConfig.getProperty("server.root")
													+ _serverConfig.getProperty("doc.root") + "/" + temp,
											_serverConfig.getProperty("default.doc"));
	...
	public void checkHttpVer(String[] rq) throws IOException {
		...
		if (rq[2] == null) {
			...
			httpVer = "1.0";
		} else if ((rq[2].startsWith("HTTP/") && (!(rq[2].compareTo("HTTP/1.0") == 0))
				&& (!(rq[2].compareTo("HTTP/0.9") == 0)))) {
			httpVer = "1.0";
	...
	public void parseHeaders(Vector v) {
		...
		// Method which handles and parses the additional headers in the request and
		// stores them in Env Variables
	...
	public void handleCGI(String fname, String cgiVars) throws IOException {
		...
		if ((f.isFile()) && (f.canRead())) {
			...
			if (methodType.equals("GET")) {
				...
				// if post request must get the body of the request and store it after
	...
	public void sendCGI(Vector c) {
		...
		/*
		 * for(int i=0; i<c.size(); i++){ tmp[i] = (String) c.elementAt(i); s = s+
		 * tmp[i]; System.out.println("adding");
		 * 
		 * try{ System.out.println("Sending"); tempb = s.getBytes();
		 * dataToClient.write(tempb); }
		 * 
		 * 
		 * catch(IOException e){} } _s.close();
		 */
		try {
	...
	public static void main(String[] args)
			throws FileNotFoundException, IOException, InterruptedException, NoSuchMethodError {
		...
		// serverConfig.load(new FileInputStream(new
		// File(".").getAbsolutePath()+"/srm.conf"));
		// mimeTypes.load(new FileInputStream(new
		// File(".").getAbsolutePath()+"/mime.types"));
		serverConfig
				.load(new FileInputStream(new File("src/org/jmule2/middleware/http").getAbsolutePath() + "/srm.conf"));
		mimeTypes.load(
				new FileInputStream(new File("src/org/jmule2/middleware/http").getAbsolutePath() + "/mime.types"));
		int localport = -1;

		boolean error = false;
		int defport = new Integer(serverConfig.getProperty("port")).intValue();
		String usage = "usage: java working_server [-p]\noptions:\n[-p port number] the port to run on (default port: "
				+ defport + ")";
		 ...
		 * 
		 * 
		 * if (localport == -1){ localport = new
		 * Integer(serverConfig.getProperty("port")).intValue(); }
		 */
