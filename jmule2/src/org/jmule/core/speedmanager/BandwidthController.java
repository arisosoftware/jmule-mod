 ...
 * current/avg. bandwith tracker. Bandwidth generally does not exceed set value
 * within a one second period. Excess bandwidth is partially available for the
 ...
 * @version $$Revision: 1.2 $$ Last changed by $$Author: binary256_ $$ on
 *          $$Date: 2008/09/02 18:08:57 $$
 */
public class BandwidthController {
	 ...
	 *            the name of this BandwidthController.
	 * @param throttlingRate
	 *            the used throttling rate in bytes per second.
	 */
	private BandwidthController(String controllerName, long throttlingRate) {
		this.controllerName = controllerName + " " + Integer.toHexString(hashCode());
	...
	public void setThrottlingRate(long bytesPerSecond) {
		...
		if (bytesPerIteration == 0)
			bytesPerIteration = 9999;
	...
	/**
	 * Returns a BandwidthController object which can be used as a bandwidth tracker
	 * and throttle.
	 * 
	 * @param controllerName
	 *            the name of BandwidthController to create.
	 * @param throttlingRate
	 *            the used throttling rate in bytes per second.
	 */
	 ...
	 * @return
	 */
	public int getAvailableByteCount(int maxToSend, boolean blockTillAvailable) {
	...
	private void updateIteration(boolean blockTillAvailable) {
		...
		while (true) {
			...
			try {
				Thread.sleep(Math.max(MILLIS_PER_ITERATION - elapsedIterationMillis, 0));
