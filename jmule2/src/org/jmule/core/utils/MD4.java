...
/**
 * Use this class for getting a MD4 message digest. Create a MD4 and reuse it
 * after a message digest calculation. There can be as many MD4 objects as you
 * want to have multiple calculations same time. The message can be passed in
 * one or a sequenze of parts wrapped in a ByteBuffer to the update of the same
 * MD4 instance. To finish the calculation use final, it will reset the MD4
...
public final class MD4 {
	...
	/**
	 * Starts or continues a MD4 message digest calculation. input.remaining()
	 * should be a multiple of 64 to be most efficant, but other amounts work too.
	 * Only remaining bytes of the ByteBuffer are used and input.position() will be
	 * input.limit() after return.
	 * 
	 * @param input
	 *            hold a part of the message. input.order() have to be
	...
	public void update(ByteBuffer input) {
		...
		if (inputLen >= partLen) {
			...
			// beim vorherigen Aufruf war und das MD4-Obj.nicht zur�ckgesetzt wurde
	...
	/**
	 * Finishs a MD4 message digest calculation. The result is stored in digest and
	 * the MD4-object is <b>reset</b> and so ready for a new message digest
	 * calculation.
	 *
	 * @param digest
