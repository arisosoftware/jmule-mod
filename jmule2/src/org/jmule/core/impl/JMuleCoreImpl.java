 ...
 * @author javajox
 * @author binary256
 * @version $$Revision: 1.29 $$ Last changed by $$Author: binary255 $$ on
 *          $$Date: 2011/04/04 12:29:12 $$
 */
public class JMuleCoreImpl implements InternalJMuleCore {
	...
	private JMuleCoreImpl() {
		// when the JMule core is created we must exactly know if this is the first
		// start up
	}
	...
	public void start() throws JMuleCoreException {
		...
		for (File file : main_dirs) {
			...
			if (!file.isDirectory())
				throw new JMuleCoreException("The file " + incoming_dir + " is not a directory");
		...
		try {
			FileHandler fileHandler = new FileHandler(ConfigurationManager.LOGS_DIR + File.separator + "JMule%u.log",
					(int) ConfigurationManager.LOG_FILE_SIZE, ConfigurationManager.LOG_FILES_NUMBER);
		 ...
		 * 
		 * public void xrun() { try { JMuleCoreImpl.this.stop(); } catch(Throwable t) {
		 * t.printStackTrace(); } } });
		 */
