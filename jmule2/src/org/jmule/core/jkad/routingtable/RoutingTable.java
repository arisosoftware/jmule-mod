 ...
 * 
 * @author binary256
 * @version $Revision: 1.19 $ Last changed by $Author: binary255 $ on $Date:
 *          2010/10/23 05:50:17 $
 */
public class RoutingTable {
			...
			public void run() {
				if (getTotalContacts() < ROUTING_TABLE_DIFICIT_CONTACTS) {
					if ((lookup_new_contacts == null) || (!lookup_new_contacts.isLookupStarted())) {
						...
						lookup_new_contacts = new LookupTask(RequestType.FIND_NODE, fake_target,
								JKadConstants.LOOKUP_NODE_TIMEOUT) {
				...
				for (MaintenanceContact maintenance_contact : maintenanceContacts.values()) {
					...
					if (contact_time >= ROUTING_TABLE_CONTACT_TIMEOUT) {
						...
						if (rcount <= contact.getContactType().toByte() + 1) {
							...
							_network_manager.sendKadPacket(hello_packet, maintenance_contact.kadContact.getIPAddress(),
									maintenance_contact.kadContact.getUDPPort());
				...
				if (maintenanceContacts.size() >= ROUTING_TABLE_MAX_MAINTENANCE_CONTACTS)
					return;
				...
				for (int i = 0; i < ROUTING_TABLE_MAINTENANCE_CONTACTS; i++) {
					...
					try {
						hello_packet = PacketFactory.getHelloReq2Packet(TagList.EMPTY_TAG_LIST);
						_network_manager.sendKadPacket(hello_packet, addContact.getIPAddress(),
								addContact.getUDPPort());
					} catch (JMException e) {
						e.printStackTrace();
					}
					c.requestCount++;
			...
			public void run() {
				for (KadContact contact : tree_nodes) {
					...
					if (contact_time > Active2MoreHours.timeRequired()) {
						if (contact.getContactType() == Active2MoreHours)
							continue;
						notifyContactUpdated(contact);
						contact.setContactType(Active2MoreHours);
					} else if (contact_time > Active1Hour.timeRequired()) {
						if (contact.getContactType() == Active1Hour)
							continue;
						notifyContactUpdated(contact);
						contact.setContactType(Active1Hour);
					} else if (contact_time > Active.timeRequired()) {
						if (contact.getContactType() == Active)
							continue;
						notifyContactUpdated(contact);
						contact.setContactType(Active);
					} else if (contact_time > JustAdded.timeRequired()) {
						if (contact.getContactType() == JustAdded)
							continue;
						notifyContactUpdated(contact);
						contact.setContactType(JustAdded);
					} else if (contact_time > ScheduledForRemoval.timeRequired()) {
						if (contact.getContactType() == ScheduledForRemoval)
							continue;
						notifyContactUpdated(contact);
	...
	public void addContact(KadContact contact) {
		...
		if (!Utils.isGoodAddress(contact.getIPAddress())) {
			...
			// Logger.getSingleton().logMessage("Filtered address :
			// "+contact.getIPAddress());
			return;
		}
		if (contact.getUDPPort() == 4666)
			return;
		if (hasContact(contact)) {
			return;
		}
		if (getTotalContacts() >= MAX_CONTACTS)
			return;
	...
	public List<KadContact> getNearestContacts(Int128 searchTarget, int contactCount) {
		...
		while (result.size() < contactCount) {
			...
			/*
			 * if (leftNode!=null) System.out.println("leftNode : " +
			 * leftNode.getLevel()+"\n");
			 * 
			 * if (rightNode!=null) System.out.println("rightNode : " +
			 * rightNode.getLevel()+"\n");
			 * 
			 * if (parentNode!=null) System.out.println("parentNode : " +
			 * parentNode.getLevel()+"\n");
			 */
