...
public class Indexer {
	...
	public int addKeywordSource(Int128 keywordID, Source source) {
		...
		if (index.getSourceCount() < JKadConstants.KEYWORD_MAX_SOURCES)
			return 100;
		if ((!index.containsClientID(source.getClientID()))
				|| (index.getSourceCount() <= JKadConstants.KEYWORD_MAX_SOURCES_FOR_NEW_SOURCES)) {
			index.addSource(source);
	...
	public Collection<Source> getFileSources(Int128 targetID) {
		...
		if (SharingManagerSingleton.getInstance().hasFile(fileHash)) {
			...
			tagList.addTag(new IntTag(JKadConstants.TAG_SOURCEIP,
					Convert.byteToInt(_jkad_manager.getIPAddress().getAddress())));
			try {
			...
			try {
				tagList.addTag(
						new ShortTag(JKadConstants.TAG_SOURCEUPORT, Convert.intToShort(config_manager.getUDP())));
			} catch (ConfigurationManagerException e) {
				e.printStackTrace();
				_jkad_manager.disconnect();
			}
