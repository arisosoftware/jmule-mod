	...
	public void processBootStrap1Res(List<KadContact> contactList) {
		for (KadContact contact : contactList) {
			if (!_routing_table.hasContact(contact)) {
				...
				// Packet packet = PacketFactory.getRequestPacket(FIND_NODE,
				// contact.getContactID(), Kad.getSingelton().getClientID());
				// udpConnection.sendPacket(packet, contact.getIPAddress(),
				// contact.getUDPPort());
			}
		}
	}
