 ...
 * 
 * @author javajox
 * @version $$Revision: 1.24 $$ Last changed by $$Author: binary255 $$ on
 *          $$Date: 2010/07/31 12:56:46 $$
 */
public class ConfigurationManagerImp extends JMuleAbstractManager implements InternalConfigurationManager {
	...
	public void setDownloadBandwidth(long downloadBandwidth) throws ConfigurationManagerException {
		if (downloadBandwidth <= 0)
			throw new ConfigurationManagerException(
					"Download bandwidth can't be negative or 0, " + downloadBandwidth + " given");
		...
		while (true) {
			...
			try {
				...
				if (file_name == null) {
					...
					if (shared_directories.isEmpty())
						shared_directories = null;
	...
	public void setUploadBandwidth(long uploadBandwidth) throws ConfigurationManagerException {
		if (uploadBandwidth <= 0) {
			throw new ConfigurationManagerException(
					"Upload bandwidth can't be negative or 0, " + uploadBandwidth + " given");
		}
	...
	private void genSecurityKeys() throws Throwable {
		...
		/*
		 * KeyPair keyPair = null; KeyPairGenerator keyGenerator =
		 * KeyPairGenerator.getInstance("RSA","BC");
		 * keyGenerator.initialize(E2DKConstants.SECURITY_IDENTIFICATION_BIT_KEY_LENGTH)
		 * ; while(true) { keyPair = keyGenerator.generateKeyPair(); RSAPublicKey key =
		 * (RSAPublicKey) keyPair.getPublic(); if (!(key.getEncoded().length >
		 * E2DKConstants.MAX_SECURITY_KEY_LENGTH)) break; } publicKey = (RSAPublicKey)
		 * keyPair.getPublic(); privateKey = (RSAPrivateKey) keyPair.getPrivate();
		 */
	}
	...
	private void loadSecurityKeys() throws Throwable {
		...
		JDKKeyFactory.RSA rsa_key_factory = new JDKKeyFactory.RSA();
		privateKey = (RSAPrivateKey) rsa_key_factory.engineGeneratePrivate(
				new RSAPrivateKeySpec(new BigInteger(modulus.array()), new BigInteger(private_exponent.array())));
		publicKey = (RSAPublicKey) rsa_key_factory.engineGeneratePublic(
				new RSAPublicKeySpec(new BigInteger(modulus.array()), new BigInteger(public_exponent.array())));

		/*
		 * KeyFactory keyFactory = KeyFactory.getInstance("RSA","BC");
		 * 
		 * publicKey = (RSAPublicKey) keyFactory .generatePublic(new
		 * RSAPublicKeySpec(new BigInteger(modulus.array()) , new
		 * BigInteger(public_exponent.array())));
		 * 
		 * privateKey = (RSAPrivateKey)keyFactory.generatePrivate(new
		 * RSAPrivateKeySpec(new BigInteger(modulus.array()) , new
		 * BigInteger(private_exponent.array())));
		 */
	}
	...
	private void notifyPropertyChanged(String property, Object new_value) {
		...
		for (ConfigurationListener listener : config_listeners) {
			...
			try {
				if (property == NICK_NAME_KEY)
					listener.nickNameChanged((String) new_value);

				else if (property == TCP_PORT_KEY)
					listener.TCPPortChanged((Integer) new_value);

				else if (property == UDP_PORT_KEY)
					listener.UDPPortChanged((Integer) new_value);

				else if (property == DOWNLOAD_BANDWIDTH_KEY)
					listener.downloadBandwidthChanged((Long) new_value);

				else if (property == UPLOAD_BANDWIDTH_KEY)
					listener.uploadBandwidthChanged((Long) new_value);

				else if (property == UDP_ENABLED_KEY)
					listener.isUDPEnabledChanged((Boolean) new_value);

				else if (property == SHARED_DIRECTORIES_KEY)
					listener.sharedDirectoriesChanged((List<File>) new_value);

				else if (property == WORKING_DIR_KEY)
					listener.workingDirChanged((File) new_value);

				else if (property == DOWNLOAD_LIMIT_KEY)
					listener.downloadLimitChanged((Long) new_value);

				else if (property == UPLOAD_LIMIT_KEY)
					listener.uploadLimitChanged((Long) new_value);

				else if (property == JKAD_ENABLED_KEY)
					listener.jkadStatusChanged((Boolean) new_value);

				else if (property == SERVER_LIST_UPDATE_ON_CONNECT_KEY)
					listener.updateServerListAtConnectChanged((Boolean) new_value);

				else if (property == JKAD_ID_KEY)
					listener.jkadIDChanged((ClientID) new_value);

				else if (property == NIC_NAME_KEY)
					listener.nicNameChanged((String) new_value);

				else if (property == NIC_IP_KEY)
					listener.nicIPChanged((String) new_value);

				else if (property == SECURITY_IDENTIFICATION)
					listener.securityIdentificationStatusChanged((Boolean) new_value);
