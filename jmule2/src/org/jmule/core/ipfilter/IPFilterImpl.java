...
public class IPFilterImpl extends JMuleAbstractManager implements InternalIPFilter {
	...
	private void addPeer(InetAddress address, BannedReason bannedReason, int howLong, TimeUnit timeUnit, String who) {

		try {
			if (timeUnit == TimeUnit.INFINITY)

				banned_peers.add(new BannedIP(AddressUtils.addressToInt(address), bannedReason, who));
			else

				temporary_banned_peers.add(new TemporaryBannedIP(AddressUtils.addressToInt(address), bannedReason,
						toMiliseconds(howLong, timeUnit), who));
	...
	private void addServer(InetAddress address, BannedReason bannedReason, int howLong, TimeUnit timeUnit, String who) {

		try {
			if (timeUnit == TimeUnit.INFINITY)

				banned_servers.add(new BannedIP(AddressUtils.addressToInt(address), bannedReason, who));
			else

				temporary_banned_servers.add(new TemporaryBannedIP(AddressUtils.addressToInt(address), bannedReason,
						toMiliseconds(howLong, timeUnit), who));
	...
	public void addPeer(InetSocketAddress address, BannedReason bannedReason, int howLong, TimeUnit timeUnit,
			String who) {
	...
	public void addPeer(InetSocketAddress inetSocketAddress, BannedReason bannedReason, int howLong,
			TimeUnit timeUnit) {
	...
	public void addServer(InetSocketAddress address, BannedReason bannedReason, int howLong, TimeUnit timeUnit,
			String who) {

		this.addServer(address.getAddress(), bannedReason, howLong, timeUnit, who);
	}
	...
	public boolean isPeerBanned(String address) {
		try {
			...
			return banned_peers.contains(bp) || temporary_banned_peers.contains(bp);
		} catch (Throwable cause) {
			cause.printStackTrace();
		}
		return false;
	...
	public boolean isServerBanned(String address) {
		try {
			...
			return banned_servers.contains(bs) || temporary_banned_servers.contains(bs);
		} catch (Throwable cause) {
			cause.printStackTrace();
		}
		return false;
	...
	private static long toMiliseconds(int howLong, TimeUnit timeUnit) {
		switch (timeUnit) {
		case MINUTE:
			return (howLong * 1000 * 60);
		case HOUR:
			return (howLong * 1000 * 60 * 60);
		case DAY:
			return (howLong * 1000 * 60 * 60 * 24);
		}
		return 0;
	}
	...
	public void addPeer(String address, BannedReason bannedReason, int howLong, TimeUnit timeUnit, String who) {
		try {
			...
			this.addPeer(InetAddress.getByAddress(AddressUtils.textToNumericFormat(address)), bannedReason, howLong,
					timeUnit, who);
		} catch (Throwable cause) {
			cause.printStackTrace();
		}
	...
	public void addServer(String address, BannedReason bannedReason, int howLong, TimeUnit timeUnit, String who) {
		try {
			...
			this.addServer(InetAddress.getByAddress(AddressUtils.textToNumericFormat(address)), bannedReason, howLong,
					timeUnit, who);
		} catch (Throwable cause) {
	...
	public void addPeer(String address, BannedReason bannedReason, int howLong, TimeUnit timeUnit, JMuleManager who) {

		this.addPeer(address, bannedReason, howLong, timeUnit, getJMuleManagerAsString(who));
	}

	public void addPeer(String address, BannedReason bannedReason, JMuleManager who) {

		this.addPeer(address, bannedReason, getJMuleManagerAsString(who));
	}

	public void addPeer(String address, int howLong, TimeUnit timeUnit, JMuleManager who) {

		this.addPeer(address, howLong, timeUnit, getJMuleManagerAsString(who));
	}
	...
	public void addServer(String address, BannedReason bannedReason, int howLong, TimeUnit timeUnit, JMuleManager who) {

		this.addServer(address, bannedReason, howLong, timeUnit, getJMuleManagerAsString(who));
	}

	public void addServer(String address, BannedReason bannedReason, JMuleManager who) {

		this.addServer(address, bannedReason, getJMuleManagerAsString(who));
	}

	public void addServer(String address, int howLong, TimeUnit timeUnit, JMuleManager who) {

		this.addServer(address, howLong, timeUnit, getJMuleManagerAsString(who));
	}
