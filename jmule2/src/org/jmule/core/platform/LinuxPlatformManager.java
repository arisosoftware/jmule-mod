...
public class LinuxPlatformManager extends UnixPlatformManager {
	...
	public List<CPUCapabilities> getCPUCapabilities() throws PlatformManagerException {
		...
		try {
			...
			BufferedReader cat_proc_cpuinfo_cmd_result = new BufferedReader(
					new InputStreamReader(cat_proc_cpuinfo_cmd_running.getInputStream()));
			String line;
			while ((line = cat_proc_cpuinfo_cmd_result.readLine()) != null) {
				...
				if (splitted_line.length == 2) {
					...
					if (key.equals("processor")) {
						...
						cpu_capabilities.setVendorId(value);
					else if (key.equals("cpu family"))
						cpu_capabilities.setFamily(value);
					else if (key.equals("model"))
						cpu_capabilities.setModel(value);
					else if (key.equals("model name"))
						cpu_capabilities.setModelName(value);
					else if (key.equals("stepping"))
						cpu_capabilities.setStepping(value);
					else if (key.equals("cpu MHz"))
						cpu_capabilities.setMHz(value);
					else if (key.equals("cache size"))
						cpu_capabilities.setCacheSize(value);
					else if (key.equals("physical id"))
						cpu_capabilities.setPhysicalId(value);
					else if (key.equals("siblings"))
						cpu_capabilities.setSiblings(value);
					else if (key.equals("core id"))
						cpu_capabilities.setCoreId(value);
					else if (key.equals("cpu cores"))
						cpu_capabilities.setNrOfCores(value);
					else if (key.equals("apicid"))
						cpu_capabilities.setApicid(value);
					else if (key.equals("initial apicid"))
						cpu_capabilities.setInitialApicid(value);
					else if (key.equals("fdiv_bug"))
						cpu_capabilities.setFdivBug(value.equals("no") ? false : true);
					else if (key.equals("hlt_bug"))
						cpu_capabilities.setHltBug(value.equals("no") ? false : true);
					else if (key.equals("f00f_bug"))
						cpu_capabilities.setF00fBug(value.equals("no") ? false : true);
					else if (key.equals("coma_bug"))
						cpu_capabilities.setComaBug(value.equals("no") ? false : true);
					else if (key.equals("fpu"))
						cpu_capabilities.setFpu(value.equals("no") ? false : true);
					else if (key.equals("fpu_exception"))
						cpu_capabilities.setFpuException(value.equals("no") ? false : true);
					else if (key.equals("cpuid level"))
						cpu_capabilities.setCpuidLevel(value);
					else if (key.equals("wp"))
						cpu_capabilities.setWp(value.equals("no") ? false : true);
					else if (key.equals("flags"))
						cpu_capabilities.setFlags(value.split(" "));
					else if (key.equals("bogomips"))
						cpu_capabilities.setBogomips(value);
					else if (key.equals("clflush size"))
						cpu_capabilities.setClflushSize(value);
					else if (key.equals("power management"))
						cpu_capabilities.setPowerManagerment(value);
				}
			}
	...
	public PingResult ping(InetAddress source, NetworkInterface networkInterface, int count)
		...
		int exit_status;
		try {
			ProcessBuilder ping_cmd = new ProcessBuilder(
					new String[] { "ping", "-I " + networkInterface.getName(), "-c " + count });
			Process ping_cmd_running = ping_cmd.start();
			exit_status = ping_cmd_running.waitFor();
			BufferedReader ping_cmd_result = new BufferedReader(
					new InputStreamReader(ping_cmd_running.getInputStream()));
			String line;
