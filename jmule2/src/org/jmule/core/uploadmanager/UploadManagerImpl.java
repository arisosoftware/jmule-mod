 ...
 * 
 * @author binary256
 * @version $$Revision: 1.35 $$ Last changed by $$Author: binary255 $$ on
 *          $$Date: 2010/08/25 12:29:39 $$
 */
public class UploadManagerImpl extends JMuleAbstractManager implements InternalUploadManager {
		...
		JMuleCoreStats.registerProvider(types, new JMuleCoreStatsProvider() {
			public void updateStats(Set<String> types, Map<String, Object> values) {
				if (types.contains(JMuleCoreStats.ST_NET_SESSION_UPLOAD_BYTES)) {
					/*
					 * long total_uploaded_bytes = 0; for(UploadSession session :
					 * session_list.values()) { total_uploaded_bytes+=session.getTransferredBytes();
					 * }
					 */
	...
	public void start() {
		...
		JMTimerTask frozen_peers_remover = new JMTimerTask() {
			public void run() {
				...
				for (UploadQueueContainer container : upload_queue.upload_queue.values()) {
					if (container.getLastResponseTime() >= ConfigurationManager.UPLOADQUEUE_REMOVE_TIMEOUT) { // if peer
																												// don't
																												// respond
																												// 1H -
																												// drop
			...
			public void run() {
				for (UserHash hash : payload_peers.payload_loosed_peers.keySet()) {
					if (System.currentTimeMillis()
							- payload_peers.payload_loosed_peers.get(hash) > ED2KConstants.TIME_24_HOURS)
		...
		maintenance_tasks.addTask(transferred_bytes_updater, ConfigurationManager.UPLOAD_QUEUE_TRANSFER_CHECK_INTERVAL,
				true);
		maintenance_tasks.addTask(payload_peers_monitor, ConfigurationManager.UPLOAD_QUEUE_PAYLOAD_CHECK_INTERVAL,
				true);
		maintenance_tasks.addTask(payload_loosed_cleaner,
				ConfigurationManager.UPLOAD_QUEUE_PAYLOAD_LOOSED_CHECK_INTERVAL, true);
	...
	public UploadSession getUpload(FileHash fileHash) throws UploadManagerException {
		if (!hasUpload(fileHash))
			throw new UploadManagerException("Upload " + fileHash + " not found");
	...
	public void removePeer(Peer peer) {
		...
		if (session != null) {
			...
			System.out.println(" uploadManager :: removePeer :: " + System.currentTimeMillis() + " :: " + peer
					+ " session peer_count :: " + session.getPeerCount());
	...
	public void receivedFileStatusRequestFromPeer(Peer sender, FileHash fileHash) {
		...
		if (shared_file instanceof PartialFile) {
			...
			_network_manager.sendFileStatusAnswer(sender.getIP(), sender.getPort(), shared_file.getHashSet(),
					shared_file.length(), partialFile.getGapList());
		} else {
			_network_manager.sendFileStatusAnswer(sender.getIP(), sender.getPort(), shared_file.getHashSet(),
					shared_file.length(), new GapList());
		}
	}
	...
	public void receivedFileRequestFromPeer(Peer sender, FileHash fileHash) {
		...
		_network_manager.sendFileRequestAnswer(sender.getIP(), sender.getPort(), shared_file.getFileHash(),
				shared_file.getSharingName());
	}
	...
	public void receivedSlotRequestFromPeer(Peer sender, FileHash fileHash) {
		...
		/*
		 * if (!uploadQueue.hasPeer(sender)) { try { uploadQueue.addPeer(sender,
		 * fileHash); recalcSlotPeers(); } catch (UploadQueueException e) { // queue is
		 * full e.printStackTrace(); return ; } }
		 */
		...
		if (upload_queue.hasSlotPeer(sender)) {
			...
			try {
				_network_manager.sendQueueRanking(sender.getIP(), sender.getPort(),
						upload_queue.getPeerPosition(sender));
			} catch (UploadQueueException e) {
				e.printStackTrace();
			}
	}
