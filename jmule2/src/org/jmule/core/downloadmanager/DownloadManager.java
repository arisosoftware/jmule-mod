 ...
 * @author javajox
 * @author binary256
 * @version $$Revision: 1.49 $$ Last changed by $$Author: binary255 $$ on
 *          $$Date: 2011/04/25 11:20:51 $$
 */
public class DownloadManager extends JMuleAbstractManager  implements JMuleCoreComponent  {
	...
	// this data structure is the helper for addAndStartSilentDownloads and
	// addAndStartSilentDownloads methods
	...
	public void initialize() {
		...
		server_sources_query = new JMTimerTask() {
			...
			public void run() {
				...
				while ((!server_sources_queue.isEmpty())
						&& (sessions_to_request.size() < ED2KConstants.SERVER_FILE_SOURCES_QUERY_ITERATION)) {
				...
				for (FileHash hash : sessions_to_request) {
					...
					try {
						...
						_network_manager.requestSourcesFromServer(session.getFileHash(),
								session.getSharedFile().length());
					} catch (DownloadManagerException e) {
						e.printStackTrace();
					}
				}
		...
		global_source_search_task = new JMTimerTask() {
			...
			public void run() {
				...
				while ((!global_sources_queue.isEmpty())
						&& (hash_list.size() < ED2KConstants.GLOBAL_FILE_SOURCES_QUERY_ITERATION)) {
				...
				for (Server server : server_list) {
					if (!server.getFeatures().contains(ServerFeatures.GetSources2))
						_network_manager.sendServerUDPSourcesRequest(server.getAddress(), server.getUDPPort(),
								hash_list);
					else if (server.getFeatures().contains(ServerFeatures.GetSources2))
						_network_manager.sendServerUDPSources2Request(server.getAddress(), server.getUDPPort(),
								hash_list, size_list);
				}
						...
						search.searchSources(search_id, new SearchResultListener() {
							public void processNewResults(List<Source> result) {
								...
								for (Source source : result) {
									...
									if (tcpPort == -1)
										continue;
									if (download_session.hasPeer(address, tcpPort))
										continue;
									Peer peer;
			...
			public void run() {
				while (true) {
					...
					if (session.getPeerCount() > ED2KConstants.RARE_SHARED_SOURCES_FILE) {
						...
						List<Peer> session_peers = session.download_status_list.getPeersByStatus(
								PeerDownloadStatus.IN_QUEUE, PeerDownloadStatus.ACTIVE,
		...
		download_peers_monitor = new JMTimerTask() {
			...
			public void run() {
				...
				for (DownloadSession session : session_list.values()) {
					List<Peer> status_not_reponse_list = session.download_status_list.getPeersWithInactiveTime(
							PEER_RESEND_PACKET_INTERVAL, PeerDownloadStatus.FILE_STATUS_REQUEST);
					...
					List<Peer> frenzed_list = session.download_status_list
							.getPeersWithInactiveTime(PEER_RESEND_PACKET_INTERVAL, ACTIVE);
					for (Peer peer : frenzed_list) {
						session.file_request_list.remove(peer);
						session.fileChunkRequest(peer);
					}
					List<Peer> peer_list = session.download_status_list.getPeersWithInactiveTime(UNUSED_PEER_ACTIVATION,
							PeerDownloadStatus.ACTIVE_UNUSED);
					...
					List<Peer> drop_queue_peer = session.download_status_list
							.getPeersWithInactiveTime(QUEUE_PEERS_DROP_TIMEOUT, PeerDownloadStatus.IN_QUEUE);
					for (Peer peer : drop_queue_peer) {
						session.removePeer(peer);
					}

					List<Peer> check_queue_position = session.download_status_list
							.getPeersWithLastSeenTime(QUEUE_PEERS_CHECK_TIMEOUT, PeerDownloadStatus.IN_QUEUE);
	...
	public void addDownload(ED2KFileLink fileLink) throws DownloadManagerException {
		if (hasDownload(fileLink.getFileHash()))
			throw new DownloadManagerException("Download " + fileLink.getFileHash() + " already exists");
	...
	public void addDownload(PartialFile partialFile) throws DownloadManagerException {
		if (hasDownload(partialFile.getFileHash()))
			throw new DownloadManagerException("Download " + partialFile.getFileHash() + " already exists");
	...
	public void cancelDownload(FileHash fileHash) throws DownloadManagerException {
		if (!hasDownload(fileHash))
			throw new DownloadManagerException("Download " + fileHash + " not found ");
	...
	public void startDownload(FileHash fileHash) throws DownloadManagerException {
		...
		// _network_manager.requestSourcesFromServer(download_session.getFileHash(),
		// download_session.getSharedFile().length());
	...
	public void peerConnectingFailed(Peer peer, Throwable cause) {
		for (DownloadSession session : session_list.values())
			if (session.hasPeer(peer)) {
				...
				} catch (Throwable fail_cause) {
					fail_cause.printStackTrace();
				}
				return;
			}
	}
	...
	public void receivedCompressedFileChunk(Peer sender, FileHash fileHash, FileChunk compressedFileChunk) {
		...
		session.receivedCompressedFileChunk(sender, compressedFileChunk);
	}
	...
	public void receivedFileStatusResponseFromPeer(Peer sender, FileHash fileHash, JMuleBitSet partStatus) {
		...
		session.receivedFileStatusResponseFromPeer(sender, fileHash, partStatus);
	}
