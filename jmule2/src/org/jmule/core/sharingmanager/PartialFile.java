...
public class PartialFile extends SharedFile {
	...
	public PartialFile(String fileName, long fileSize, FileHash fileHash, PartHashSet hashSet, TagList tagList)
	...
	public String toString() {
		...
		return "[ " + partFile.getName() + " " + this.length() + " Completed : " + Math.round(getPercentCompleted())
				+ " % GapList : " + partFile.getGapList() + " ] ";
	...
	public void writeData(FileChunk fileChunk) throws SharedFileException {
		synchronized (lock) {
			try {
				...
				} else if ((writeChannel.size() > fileChunk.getChunkStart())
						&& (writeChannel.size() <= fileChunk.getChunkEnd())) {
				...
				partFile.getGapList().removeGap(fileChunk.getChunkStart(),
						fileChunk.getChunkStart() + fileChunk.getChunkData().capacity());
				...
				if (t instanceof java.io.IOException) {
					JMuleCoreFactory.getSingleton().getDownloadManager().stopDownload();
					((InternalJMuleCore) JMuleCoreFactory.getSingleton()).notifyListenersEventOccured(
							JMuleCoreEvent.NOT_ENOUGH_SPACE, new NotEnoughSpaceDownloadingFile(file.getName(),
	 ...
	 * 
	 * @return list with ID's of broken parts, -1 - file hash not match, -2 - don't
	 *         have hash set, -3 - hash set size not match
	 */
	public List<Integer> checkFullFileIntegrity() {
		...
		for (int i = 0; i < fileHashSet.size(); i++) {
			...
			if (!Arrays.equals(a, b)) {
				...
				/*
				 * //Adding gap this.partFile.getGapList().addGap(begin,end); return false;
				 */
			}
		}
	...
	public boolean hasHashSet() {
		if (length() < ED2KConstants.PARTSIZE)
			return true;
