 ...
 * the Group class. GroupLayout supports two types of groups:
 * <table>
 * <tr>
 ...
 * <td>A parallel group positions its child elements in the same space on top of
 * each other. Parallel groups can also align the child elements along their
 * baseline.
 * </table>
 ...
 * vertical axis. The horizontal group is responsible for setting the x and
 * width of its contents, where as the vertical group is responsible for setting
 * the y and height of its contents.
 * <p>
 ...
 * <li>The vertical axis similarly consists of a sequential group containing two
 * parallel groups. The parallel groups align their contents along the baseline.
 * The first parallel group consists of the first label and text field, and the
 * second group consists of the second label and text field.
 * </ul>
...
public class GroupLayout implements LayoutManager2 {
	 ...
	 * origin. For the horizontal axis with a left to right orientation this means
	 * aligned to the left.
	 *
	...
	/**
	 * Possible alignment type. Indicates the elements should be aligned to the end.
	 * For the horizontal axis with a left to right orientation this means aligned
	 * to the right.
	 *
	 * @see #createParallelGroup(int)
	 */
	...
	/**
	 * Possible alignment type. Indicates the elements should aligned along their
	 * baseline.
	 *
	 * @see #createParallelGroup(int)
	 */
	...
	/**
	 * Possible value for the add methods that takes a Component. Indicates the size
	 * from the component should be used.
	 */
	...
	/**
	 * Group responsible for layout along the horizontal axis. This is NOT the user
	 * specified group, use getHorizontalGroup to dig that out.
	 */
	private Group horizontalGroup;
	/**
	 * Group responsible for layout along the vertical axis. This is NOT the user
	 * specified group, use getVerticalGroup to dig that out.
	 */
	...
	/**
	 * If true, components that are not visible are treated as though they aren't
	 * there.
	 */
	...
	/**
	 * Sets whether component visiblity is considered when sizing and positioning
	 * components. A value of <code>true</code> indicates that non-visible
	 * components should not be treated as part of the layout. A value of
	 * <code>false</code> indicates that components should be positioned and sized
	 * regardless of visibility.
	 * <p>
	 * A value of <code>false</code> is useful when the visibility of components is
	 * dynamically adjusted and you don't want surrounding components and the sizing
	 * to change.
	 * <p>
	...
	/**
	 * Returns whether component visiblity is considered when sizing and positioning
	 * components.
	 *
	 * @return whether component visiblity is considered when sizing and positioning
	 *         components
	 */
	 ...
	 * <code>component</code> is not visible it should not be treated as part of the
	 * layout. A value of <code>false</code> indicates that <code>component</code>
	 * is positioned and sized regardless of it's visibility. A value of
	 * <code>null</code> indicates the value specified by the single argument method
	 * <code>
	 ...
	 *            the component
	 * @param honorsVisibility
	 *            whether <code>component</code>'s visiblity should be considered
	...
	private void dump(StringBuffer buffer, Spring spring, String indent, int axis) {
		...
		buffer.append(indent + spring.getClass().getName() + " " + Integer.toHexString(spring.hashCode()) + " " + origin
				+ ", size=" + spring.getSize() + ", alignment=" + spring.getAlignment() + " prefs=["
				+ spring.getMinimumSize(axis) + " " + spring.getPreferredSize(axis) + " " + spring.getMaximumSize(axis)
				+ padding + "]\n");
	 ...
	 * For example, if this is true and you add two components to a
	 * <code>SequentialGroup</code> a gap between the two will automatically be
	 * created. The default is false.
	 *
	 * @param autocreatePadding
	 *            whether or not to automatically created a gap between components
	 *            and the container
	 */
	...
	/**
	 * Sets whether or not gaps between the container and the first/last components
	 * should automatically be created. The default is false.
	 *
	 * @param autocreatePadding
	 *            whether or not to automatically create gaps between the container
	 *            and first/last components.
	 */
	 ...
	 *
	 * @param group
	 *            <code>Group</code> responsible for layout along the horizontal
	 *            axis
	 * @throws IllegalArgumentException
	 *             if group is null
	 */
	...
	/**
	 * Sets the <code>Group</code> that is responsible for layout along the vertical
	 * axis.
	 *
	 * @param group
	 *            <code>Group</code> responsible for layout along the vertical axis.
	 * @throws IllegalArgumentException
	 *             if group is null.
	 */
	...
	/**
	 * Returns the <code>ParallelGroup</code> that is responsible for layout along
	 * the vertical axis.
	 *
	 * @return <code>ParallelGroup</code> responsible for layout along the vertical
	 *         axis.
	 */
	...
	/**
	 * Creates and returns a <code>ParallelGroup</code> with a <code>LEADING</code>
	 * alignment. This is a cover method for the more general
	 * <code>createParallelGroup(int)</code> method.
	 *
	...
	/**
	 * Creates and returns an <code>ParallelGroup</code>. The alignment specifies
	 * how children elements should be positioned when the the parallel group is
	 * given more space than necessary. For example, if a ParallelGroup with an
	 * alignment of TRAILING is given 100 pixels and a child only needs 50 pixels,
	 * the child will be positioned at the position 50.
	 *
	 * @param alignment
	 *            alignment for the elements of the Group, one of
	 *            <code>LEADING</code>, <code>TRAILING</code>, <code>CENTER</code>
	 *            or <code>BASELINE</code>.
	 * @throws IllegalArgumentException
	...
	/**
	 * Creates and returns a <code>ParallelGroup</code> that aligns it's elements
	 * along the baseline.
	 *
	 * @param resizable
	 *            whether the group is resizable
	 * @param anchorBaselineToTop
	 *            whether the baseline is anchored to the top or bottom of the group
	...
	/**
	 * Forces the set of components to have the same size. This can be used multiple
	 * times to force any number of components to share the same size.
	 * <p>
	 ...
	 *             if <code>components</code> is null, or contains null.
	 * @throws IllegalArgumentException
	 *             if <code>axis</code> does not contain <code>HORIZONTAL</code> or
	 *             <code>VERTICAL</code>
	 */
	...
	private void linkSize0(Component[] components, int axis) {
		LinkInfo master = getComponentInfo(components[components.length - 1]).getLinkInfo(axis);
	 ...
	 * @throws IllegalArgumentException
	 *             is either of the Components are null or if existingComponent is
	 *             not being managed by this layout manager
	 */
	...
	/**
	 * Sets the LayoutStyle this GroupLayout is to use. A value of null can be used
	 * to indicate the shared instance of LayoutStyle should be used.
	 *
	 * @param layoutStyle
	...
	/**
	 * Notification that a <code>Component</code> has been removed from the parent
	 * container. You should not invoke this method directly, instead invoke
	 * <code>remove</code> on the parent <code>Container</code>.
	 *
	 * @param component
	 ...
	 * @throws IllegalArgumentException
	 *             if <code>parent</code> is not the same <code>Container</code>
	 *             that this was created with
	 * @throws IllegalStateException
	 *             if any of the components added to this layout are not in both a
	...
	/**
	 * Notification that a <code>Component</code> has been added to the parent
	 * container. You should not invoke this method directly, instead you should use
	 * one of the <code>Group</code> methods to add a <code>Component</code>.
	 *
	 * @param component
	 *            The component added
	 * @param constraints
	...
	private void prepare(int sizeType) {
		...
		if (!isValid) {
			...
			for (Iterator cis = componentInfos.values().iterator(); cis.hasNext();) {
	...
	/**
	 * Adjusts the autopadding springs for the horizontal and vertical groups. If
	 * <code>insert</code> is true this will insert auto padding springs, otherwise
	 * this will only adjust the springs that comprise auto preferred padding
	 * springs.
	 */
	private void insertAutopadding(boolean insert) {
		horizontalGroup.insertAutopadding(HORIZONTAL, new ArrayList(1), new ArrayList(1), new ArrayList(1),
				new ArrayList(1), insert);
		verticalGroup.insertAutopadding(VERTICAL, new ArrayList(1), new ArrayList(1), new ArrayList(1),
				new ArrayList(1), insert);
	}

	/**
	 * Returns true if the two Components have a common ParallelGroup ancestor along
	 * the particular axis.
	 */
	 ...
	 * middle of that and a location. Subclasses must override methods to get the
	 * min/max/pref and will likely want to override the <code>setSize</code>
	 * method. Spring automatically caches the min/max/pref. If the min/pref/max has
	 * internally changes, or needs to be updated you must invoked clear.
	 */
	abstract class Spring {
		...
		/**
		 * Sets the value and location of the spring. Subclasses will want to invoke
		 * super, then do any additional sizing.
		 *
		 * @param axis
		...
		/**
		 * Returns true if this Spring will ALWAYS have a zero size. This should NOT
		 * check the current size, rather it's meant to quickly test if this Spring will
		 * always have a zero size.
		 */
	 ...
	 * by <code>GroupLayout</code>: laying out components one after another
	 * (<code>SequentialGroup</code>) or layout on top of each other
	 * (<code>ParallelGroup</code>). Use one of <code>createSequentialGroup</code>
	 * or <code>createParallelGroup</code> to create one.
	 */
	public abstract class Group extends Spring {
		...
		/**
		 * Adds the Spring to the list of <code>Spring</code>s and returns the receiver.
		 */
		...
		/**
		 * This is invoked from <code>setSize</code> if passed a value other than UNSET.
		 */
		 ...
		 * For example, a group that layed things out one after the next would return
		 * <code>a + b</code>.
		 */
		 ...
		 * @param insert
		 *            Whether or not to insert AutopaddingSprings or just adjust any
		 *            existing AutopaddingSprings.
		 */
		abstract void insertAutopadding(int axis, List leadingPadding, List trailingPadding, List leading,
				List trailing, boolean insert);
	...
	public class SequentialGroup extends Group {
		...
		/**
		 * Adds the specified <code>Group</code> to this <code>SequentialGroup</code>
		 *
		 * @param group
		 *            the Group to add
		 * @return this Group
		 ...
		 * @param useAsBaseline
		 *            whether the specified <code>Group</code> should be used to
		 *            calculate the baseline for this <code>Group</code>
		...
		/**
		 * Adds the specified Component. If the Component's min/max are different from
		 * its pref than the component will be resizable.
		 *
		 * @param component
		 ...
		 *
		 * @param useAsBaseline
		 *            whether the specified <code>Component</code> should be used to
		 *            calculate the baseline for this <code>Group</code>
		 * @param component
		...
		/**
		 * Adds the specified <code>Component</code>. Min, pref and max can be absolute
		 * values, or they can be one of <code>DEFAULT_SIZE</code> or
		 * <code>PREFERRED_SIZE</code>. For example, the following:
		 * 
		 * <pre>
		 * add(component, PREFERRED_SIZE, PREFERRED_SIZE, 1000);
		 * </pre>
		 * 
		 * Forces a max of 1000, with the min and preferred equalling that of the
		 * preferred size of <code>component</code>.
		 *
		 * @param component
		 *            the Component to add
		 * @param min
		...
		public SequentialGroup add(Component component, int min, int pref, int max) {
			return (SequentialGroup) addSpring(new ComponentSpring(component, min, pref, max));
		}

		/**
		 * Adds a <code>Component</code> to this <code>Group</code> with the specified
		 * size.
		 *
		 * @param useAsBaseline
		 *            whether the specified <code>Component</code> should be used to
		 ...
		 * @param pref
		 *            the preferred size or one of <code>DEFAULT_SIZE</code> or
		 *            <code>PREFERRED_SIZE</code>
		 * @param max
		 ...
		 *            the size of the gap
		 * @throws IllegalArgumentException
		 *             if min < 0 or pref < 0 or max < 0 or the following is not meant
		 *             min <= pref <= max
		 * @return this <code>SequentialGroup</code>
		 */
		 ...
		 * @param type
		 *            the type of gap; one of the constants defined by LayoutStyle
		 * @return this <code>SequentialGroup</code>
		 * @throws IllegalArgumentException
		 *             if <code>type</code> is not a valid LayoutStyle constant
		 * @see LayoutStyle
		 */
		...
		public SequentialGroup addPreferredGap(JComponent comp1, JComponent comp2, int type, boolean canGrow) {
			if (type != LayoutStyle.RELATED && type != LayoutStyle.UNRELATED && type != LayoutStyle.INDENT) {
		 ...
		 * components. That is, during layout the neighboring components are found, and
		 * the min, pref and max of this element is set based on the preferred gap
		 * between the components. If no neighboring components are found the min, pref
		 * and max are set to 0.
		 * 
		 * @param type
		 *            the type of gap; one of the LayoutStyle constants
		 * @return this SequentialGroup
		 * @throws IllegalArgumentException
		 *             if type is not one of <code>LayoutStyle.RELATED</code> or
		...
		/**
		 * Adds an element for the preferred gap between the nearest components. That
		 * is, during layout the neighboring components are found, and the min of this
		 * element is set based on the preferred gap between the components. If no
		 * neighboring components are found the min is set to 0. This method allows you
		 * to specify the preferred and maximum size by way of the <code>pref</code> and
		 * <code>max</code> arguments. These can either be a value &gt;= 0, in which
		 * case the preferred or max is the max of the argument and the preferred gap,
		 * of DEFAULT_VALUE in which case the value is the same as the preferred gap.
		 * 
		 * @param type
		 ...
		 * @param max
		 *            the maximum size; one of DEFAULT_SIZE, PREFERRED_SIZE or a value >
		 *            0
		 * @return this SequentialGroup
		 * @throws IllegalArgumentException
		 *             if type is not one of <code>LayoutStyle.RELATED</code> or
		 *             <code>LayoutStyle.UNRELATED</code> or pref/max is != DEFAULT_SIZE
		 *             and < 0, or pref > max
		 * @see LayoutStyle
		 */
		 ...
		 * container and the next/previous Component. This will have no effect if the
		 * next/previous element is not a Component and does not touch one edge of the
		 * parent container.
		 *
		 * @return this <code>SequentialGroup</code>.
		 */
		...
		void insertAutopadding(int axis, List leadingPadding, List trailingPadding, List leading, List trailing,
				boolean insert) {
			...
			while (counter < springs.size()) {
				...
				if (spring instanceof AutopaddingSpring) {
					...
					if (spring instanceof ComponentSpring) {
						...
						newLeading.clear();
						newLeadingPadding.clear();
						int nextCounter = indexOfNextNonZeroSpring(counter + 1, newTrailing.size() == 0);
						if (nextCounter == springs.size()) {
	...
	/**
	 * A <code>Group</code> that lays out its elements on top of each other. If a
	 * child element is smaller than the provided space it is aligned based on the
	 * alignment of the child (if specified) or on the alignment of the
	 * ParallelGroup.
	 *
	...
	public class ParallelGroup extends Group {
		...
		public ParallelGroup add(Component component, int min, int pref, int max) {
			return (ParallelGroup) addSpring(new ComponentSpring(component, min, pref, max));
		}
		...
		public ParallelGroup add(int alignment, Component component) {
			return add(alignment, component, DEFAULT_SIZE, DEFAULT_SIZE, DEFAULT_SIZE);
		}
		...
		private void checkChildAlignment(int alignment) {
			...
			if (alignment != CENTER && alignment != BASELINE && alignment != LEADING && alignment != TRAILING) {
				throw new IllegalArgumentException("Alignment must be one of:" + "LEADING, TRAILING or CENTER");
			}
		}
	}
	...
	private class BaselineGroup extends ParallelGroup {
		...
		private void calculateBaselineAndResizeBehavior() {
			...
			for (int counter = springs.size() - 1; counter >= 0; counter--) {
				Spring spring = getSpring(counter);
				if (spring.getAlignment() == NO_ALIGNMENT || spring.getAlignment() == BASELINE) {
		...
		/**
		 * Lays out springs that have a baseline along the baseline. All others are
		 * centered.
		 */
	 ...
	 * number.
	 * <li>Force the min/max/pref to be a certain value. If the Component's size is
	 * to be linked to another components than the min/max/pref all come from the
	 * ComponentInfo.
	 */
	private final class ComponentSpring extends Spring {
		...
		int getBaseline() {
			if (baseline == -1 && (component instanceof JComponent)) {
				...
				if (width > 0 && height > 0) {
					baseline = Baseline.getBaseline((JComponent) component, width, getPreferredSize(VERTICAL));
				}
			}
			return baseline;
		}
	...
	/**
	 * Spring reprensenting the distance between any number of sources and targets.
	 * The targets and sources are computed during layout. An instance of this can
	 * either be dynamically created when autocreatePadding is true, or explicitly
		...
		private int calculatePadding(LayoutStyle p, int position, ComponentSpring source, ComponentSpring target) {
			...
			if (delta >= 0) {
				...
				if ((source.getComponent() instanceof JComponent) && (target.getComponent() instanceof JComponent)) {
					padding = p.getPreferredGap((JComponent) source.getComponent(), (JComponent) target.getComponent(),
							type, position, host);
				} else {
					padding = 10;
				}
	...
	/**
	 * Tracks the horizontal/vertical Springs for a Component. This class is also
	 * used to handle Springs that have their sizes linked.
	 */
