 ...
 * static information about it. This information is available from the body of
 * advice using the special form <code>thisJoinPoint</code>. The primary use of
 * this reflective information is for tracing and logging applications.
 * </p>
 *
 * <pre>
 * aspect Logging {
...
public interface JoinPoint {
	...
	/**
	 * <p>
	 * Returns the currently executing object. This will always be the same object
	 * as that matched by the <code>this</code> pointcut designator. Unless you
	 * specifically need this reflective access, you should use the
	 * <code>this</code> pointcut designator to get at this object for better static
	 * typing and performance.
	 * </p>
	 *
	 * <p>
	 * Returns null when there is no currently executing object available. This
	 * includes all join points that occur in a static context.
	 * </p>
	 */
	Object getThis();
	 ...
	 * matched by the <code>target</code> pointcut designator. Unless you
	 * specifically need this reflective access, you should use the
	 * <code>target</code> pointcut designator to get at this object for better
	 ...
	 *
	 * <p>
	 * <code>getStaticPart().getSourceLocation()</code> returns the same object.
	 * </p>
	 */
	SourceLocation getSourceLocation();

	/**
	 * Returns a String representing the kind of join point. This String is
	 * guaranteed to be interned. <code>getStaticPart().getKind()</code> returns the
	 * same object.
	 */
	String getKind();
	 ...
	 * <code>thisJoinPointStaticPart</code>.
	 * </p>
	 *
	 * <p>
	 * If you are only interested in the static information about a join point, you
	 * should access it through this type for the best performance. This is
	 * particularly useful for library methods that want to do serious manipulations
	 * of this information, i.e.
	 * </p>
	 *
	 * <pre>
	...
	public interface StaticPart {
		 ...
		 * instances are assigned an id number upon creation. For each advised type the
		 * id numbers start at 0. <br>
		 * The id is guaranteed to remain constant across repeated executions of a
		 * program but may change if the code is recompiled. <br>
		 * The benefit of having an id is that it can be used for array index purposes
		 * which can be quicker than using the JoinPoint.StaticPart object itself in a
		 * map lookup. <br>
		 ...
		 * state then that state must be maintained on a pertype basis - either by using
		 * pertypewithin() or an ITD.
		 * 
